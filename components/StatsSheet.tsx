...
export function StatsSheet({ visible, files, onClose }: StatsSheetProps) {
  ...
  const stats = useMemo(() => {
    ...
    const largest = files.reduce<VaultFile | null>(
      (max, f) => (max === null || f.size > max.size ? f : max), null
    );
    const oldest  = files.reduce<VaultFile | null>(
      (min, f) => (min === null || f.createdAt < min.createdAt ? f : min), null
    );
    ...
    const thisMonth = files.filter((f) => isThisMonth(f.createdAt * 1000)).length;

    return { count, total, avg, largest, oldest, albums, favs, distinctTags, tagged, thisWeek, thisMonth };
  }, [files]);

  const largestName = stats.largest
    ? stats.largest.name.replace('.vault', '')
    : '—';
  const oldestDate  = stats.oldest ? shortDate(stats.oldest.createdAt * 1000) : '—';
    ...
    <Animated.View entering={FadeIn.duration(150)} style={styles.overlay}>
      ...
      {stats.count === 0 ? (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyDesc}>Add encrypted files to see statistics.</Text>
        </View>
      ) : (
        <ScrollView
          style={styles.scroll}
        ...
        >
          ...
          <View style={styles.card}>
            ...
            <View style={styles.divider} />
            <StatRow label="Distinct tags"  value={`${stats.distinctTags.size}`} />
            <View style={styles.divider} />
            <StatRow label="Files with tags" value={`${stats.tagged}`} />
            <View style={styles.divider} />
            <StatRow label="Untagged files" value={`${stats.count - stats.tagged}`} />
          </View>
...
const styles = StyleSheet.create({
  ...
  content: { padding: Spacing.lg, gap: Spacing.sm, paddingBottom: Spacing['3xl'] } as ViewStyle,
  ...
  emptyTitle: { fontSize: Typography.lg, fontWeight: Typography.semibold, color: Colors.textSecondary } as TextStyle,
  emptyDesc:  { fontSize: Typography.sm, color: Colors.textMuted, textAlign: 'center' } as TextStyle,
